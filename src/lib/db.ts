...
/** Mirrors the SQLite file to a single blob (`amazon.db`). */
class BlobMirror {
  ...
  async pullIfNewer(force = false): Promise<boolean> {
    ...
    try {
      ...
      const { blobs } = await list({ prefix: "amazon.db", limit: 1 });
      const b = blobs[0];
      if (!b) return false;
    ...
    this.uploading = (async () => {
      while (this.dirty) {
        ...
        try {
          ...
          const r = await put("amazon.db", fs.readFileSync(this.file), { access: "public", addRandomSuffix: false, allowOverwrite: true, contentType: "application/octet-stream" });
          try { const h = await head(r.url); this.lastSeen = new Date(h.uploadedAt).getTime(); } catch { this.lastSeen = Date.now() + 2000; }
          console.log(`[db] mirrored ${fs.statSync(this.file).size} bytes to blob`);
        } catch (e) { console.error("[db] blob upload failed", e); }
      }
      this.uploading = null;
    })();
