...
import { CATALOG_DEPARTMENTS, CATEGORY_LABEL, search, type SortKey } from "@/lib/products";
...
const SORTS: [SortKey, string][] = [["featured", "Featured"], ["price-asc", "Price: Low to High"], ["price-desc", "Price: High to Low"], ["review", "Avg. Customer Review"], ["newest", "Newest Arrivals"], ["bestsellers", "Best Sellers"]];
  ...
  return (
    <div className="bg-white">
      ...
      <div className="flex items-center justify-between border-b border-[#ddd] px-5 py-[11px] shadow-[0_1px_2px_rgba(0,0,0,.06)]">
        ...
        <form className="flex items-center gap-2 text-[12px]"><span>Sort by:</span>
          <div className="flex flex-wrap gap-1">{SORTS.map(([k, l]) => <Link key={k} href={href(sp, { sort: k })} className={`rounded-md border px-2 py-1 ${res && (sp.sort || "featured") === k ? "border-ink bg-[#f0f2f2] font-bold" : "border-line hover:bg-[#f7fafa]"} text-ink hover:no-underline`}>{l}</Link>)}</div>
        </form>
      </div>
