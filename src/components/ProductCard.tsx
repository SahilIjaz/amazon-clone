...
export function GridCard({ p, compact = false }: { p: Product; compact?: boolean }) {
  return (
    <div className={`flex flex-col ${compact ? "w-[150px]" : "w-[200px]"} shrink-0`}>
      <Link href={productUrl(p)} className="block bg-[#f7f7f7] rounded-sm p-2">
        <div className={`relative ${compact ? "h-[150px]" : "h-[200px]"}`}><Image src={p.thumb} alt={p.title} fill sizes="200px" className="object-contain mix-blend-multiply" /></div>
      </Link>
...
export function ResultCard({ p, q }: { p: Product; q?: string }) {
  ...
  return (
    <div className="s-result flex gap-4 p-4">
      <Link href={url} className="relative block h-[220px] w-[220px] shrink-0 rounded-sm bg-[#f7f7f7]"><Image src={p.thumb} alt={p.title} fill sizes="220px" className="object-contain mix-blend-multiply p-2" /></Link>
